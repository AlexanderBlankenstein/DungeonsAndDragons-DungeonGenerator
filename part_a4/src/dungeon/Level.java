...
public class Level implements Serializable {
    ...
    private static ArrayList<Chamber> chambers;
    private static ArrayList<Passage> passages;
    private static HashMap<Door, Chamber> exits;
    private static HashMap<Door, ArrayList<Chamber>> connections;
    private static ArrayList<Door> doors;
    //private ArrayList<Chamber> chambers;
    //private HashMap <Door,Chamber> exits;
    private static HashMap <Door,Door> matches;
    ...
    public void createChamberList(int numOfChambers){
        createChambers(numOfChambers);
        setConnectTargets();
        setTargets();
        makeFinalLineup();
    }
    ...
    public static HashMap<Door,Chamber> getExitsList(){
        return exits;
    }

    public static HashMap <Door,ArrayList<Chamber>> getConnections(){
        return connections;
    }

    public static String HelpConnections(Chamber chamber){
        int exitNum = 1;
        String description = "";
        for (Door door:chamber.getDoors()) {
            description += "Passage connected to exit " + exitNum + ": " + door.getSpaces().get(1).getDescription();
            exitNum++;
            for (Door connectedDoor:exits.keySet()) {
                if (door == connectedDoor){
    ...
    public static void loadLevel(File file){
        try {
            ...
            exits = (HashMap<Door, Chamber>) in.readObject();
            connections = (HashMap<Door, ArrayList<Chamber>>) in.readObject();
            doors = (ArrayList<Door>) in.readObject();
            matches = (HashMap <Door,Door>) in.readObject();
            in.close();
            fileIn.close();
